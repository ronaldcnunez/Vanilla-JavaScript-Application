document.addEventListener('DOMContentLoaded', () => {
 ...
 fetch(`https://country.register.gov.uk/records.json?page-size=5000`).
  then(r => r.json()).then(data => {
    ...
    console.log(info)
     var element = info[Math.floor(Math.random() * info.length)];
    const li_tag = document.createElement('div')
          li_tag.innerText = element.item[0].country
          li_tag.id = element.item[0].name
...
function secondRound(){
  fetch(`https://country.register.gov.uk/records.json?page-size=5000`).
  then(r => r.json()).then(data =>{ const info = Object.values(data)
    info.forEach(function(element){
      const li_tag = document.createElement('div')
         li_tag.innerText = element.item[0]["name"]
         li_tag.id = element.item[0]["citizen-name"]
         li_tag.class = "countries"
         li_tag.dataset.countryName = element.item[0].name
